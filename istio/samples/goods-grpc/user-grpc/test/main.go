...
import (
	...
	health_check "user/api/protobuf_spec/health_check"
)
...
func main() {
    ...
    // init log
    err := logger.Init("../configs/log.config", "user-grpc_test")

	if err != nil {
		log.Warnf("Init logger failed! error:%v", err.Error())
	}

	log.Info("====== Test [user-grpc] begin ======")
	defer log.Info("====== Test [user-grpc] end ======")

	// init monitor
	err = monitor.Init("127.0.0.1:8125", "user-grpc_test")
    ...
    // init tracing
    err = tracing.Init("127.0.0.1:6831", "user-grpc_test")
...
func testHealthCheck() {
	...
	for index := 0; index < 1; index++ {
		// get connection
		conn, err := pool.GetConnectionPool().GetConnection(context.Background(), "0.0.0.0:19999")
